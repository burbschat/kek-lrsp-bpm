...
class Root(pr.Root):
    def __init__(self,
        ...
        if ip != None:
            ...
            # TODO: Make sure to remove rate limiter in firmware!!!
            self.ringBufferAdc     = [stream.TcpClient(ip,10000+2*(i+4))  for i in range(4)]  # No DACs required here
        else:
            ...
            self.ringBufferAdc     = [rogue.hardware.axi.AxiStreamDma('/dev/axi_stream_dma_0', i+4,  True) for i in range(4)]  # No DACs required here
        ...
        for i in range(4):
            ...
            self.ringBufferAdc[i] >> self.dataWriter.getChannel(i+4)  # Use same channel numbers as axis TDEST for consistency
