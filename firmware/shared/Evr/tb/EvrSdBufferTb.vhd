...
entity EvrSdBufferTb is end EvrSdBufferTb;

architecture testbed of EvrSdBufferTb is
...
begin
   ...
   --------------------------
   U_DUT : entity work.EvrSdBuffer
      generic map (
         TPD_G               => TPD_C,
         COMMON_CLK_G        => false,  -- true if dataClk=axilClk
         DATA_BYTES_G        => 1,      -- 8-bit per word
         RAM_ADDR_WIDTH_G    => 11,      -- 128 = 128 byte words deep (real buffer will have 2048)
   ...
   begin
      ...
      if (r.cnt /= x"FFF") then
         ...
         v.dataSegIdx := toSlv(4, 7);

         -- Generate data. The buffer has 128 16 bit words so 128 transmissions
         -- are required to fill the buffer
         if (r.cnt < 10 + 128 * 3) and (r.cnt >= 10) then
            v.data      := r.data + 1;
            v.dataValid := '1';
         else
            -- Start at max value to wrap around to 0 in first write cycle
            v.data      := (others => '1');
            v.dataValid := '0';
         end if;
         ...
         -- frame end condition.
         if (r.cnt = 10 + 128 * 2 + 64) then
            -- Set the flag
            v.frameDone := '1';
         end if;

         -- Close out partial frame
         if (r.cnt = 400) then
            -- Set the flag
            v.frameDone := '1';
         end if;

         -- Issue readout trigger
         if (r.cnt = 512) then
            -- Set the flag
            v.dataRdTrig := '1';
         end if;
         ...
         if (r.cnt > 700) and (r.cnt < (700 + 32)) then
            -- Set the flag
            v.axilRdTrig := '1';
         end if;
